"""
State-Extended SPFA算法测试
"""

import pytest

from second_shortest_path.algorithms import StateExtendedSPFA


class TestStateExtendedSPFA:
    """测试State-Extended SPFA算法"""
    
    def test_simple_graph(self, simple_graph):
        """测试简单图"""
        algo = StateExtendedSPFA(simple_graph)
        ...
        assert stats['enqueue_operations'] > 0
        assert stats['dequeue_operations'] > 0
        assert stats['edge_relaxations'] > 0
        assert stats['iterations'] > 0
    
    def test_chain_graph(self, chain_graph):
        """测试链式图"""
        algo = StateExtendedSPFA(chain_graph)
    ...
    def test_complete_graph(self, complete_graph):
        """测试完全图"""
        algo = StateExtendedSPFA(complete_graph)
        shortest, second_shortest = algo.find_second_shortest(0, 4)
    ...
    def test_disconnected_graph(self, disconnected_graph):
        """测试非连通图"""
        algo = StateExtendedSPFA(disconnected_graph)
        shortest, second_shortest = algo.find_second_shortest(0, 4)
