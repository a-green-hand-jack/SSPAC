...
生成三张图：
1. 最短路径：Dijkstra vs SPFA
2. 次短路径：Two-Distance Dijkstra vs State-Extended SPFA
3. 综合对比
"""
...
shortest_spfa = [1.29, 3.04, 7.67, 15.89, 24.76, 45.40, 69.62]

# 次短路径数据 (ms) - 完整数据
second_dijkstra = [2.49, 5.96, 10.93, 17.90, 25.85, 38.75, 50.03]
second_spfa = [1.61, 4.31, 8.83, 21.61, 36.08, 57.39, 86.68]
...
def plot_shortest_path():
    ...
    bars2 = ax.bar(x + width/2, shortest_spfa, width,
                   label='SPFA', color='#e74c3c', edgecolor='black')

    ax.set_xlabel('Grid Size', fontsize=12)
    ax.set_ylabel('Time (ms)', fontsize=12)
    ax.set_title('Shortest Path: Dijkstra vs SPFA on Grid Graphs',
    ...
    # Dijkstra 始终更快的区域着色
    ax.fill_between([-0.5, 6.5], 0, max(shortest_spfa) * 1.15,
                    alpha=0.08, color='#2ecc71')
    ax.text(3, max(shortest_spfa) * 0.95, 'Dijkstra consistently faster',
            ha='center', fontsize=11, color='#27ae60', fontweight='bold',
            bbox=dict(boxstyle='round', facecolor='white', alpha=0.8))

    ax.set_xlim(-0.5, 6.5)
    ax.set_ylim(0, max(shortest_spfa) * 1.15)
...
def plot_second_shortest_path():
    ...
    bars2 = ax.bar(x + width/2, second_spfa, width,
                   label='State-Extended SPFA', color='#e67e22', edgecolor='black')

    ax.set_xlabel('Grid Size', fontsize=12)
    ax.set_ylabel('Time (ms)', fontsize=12)
    ax.set_title('Second Shortest Path: Two-Distance Dijkstra vs State-Extended SPFA',
                 fontsize=14, fontweight='bold')
    ax.set_xticks(x)
    ax.set_xticklabels([f'{s}×{s}\n({n:,})' for s, n in zip(grid_sizes, nodes)])
    ax.legend(loc='upper left', fontsize=11)
    ...
    ax.annotate('Crossover Point\n(~62,500 nodes)',
                xy=(crossover_x, max(second_spfa) * 0.7),
                ha='center', fontsize=10, color='red', fontweight='bold',
                bbox=dict(boxstyle='round', facecolor='white', alpha=0.9))

    # 添加区域着色
    ax.fill_between([-0.5, crossover_x], 0, max(second_spfa) * 1.15,
                    alpha=0.08, color='#e67e22')
    ax.fill_between([crossover_x, 6.5], 0, max(second_spfa) * 1.15,
                    alpha=0.08, color='#3498db')

    # 区域标签
    ax.text(1, max(second_spfa) * 0.45, 'SPFA faster',
            ha='center', fontsize=10, color='#d35400', fontweight='bold')
    ax.text(4.5, max(second_spfa) * 0.45, 'Dijkstra faster',
            ha='center', fontsize=10, color='#2980b9', fontweight='bold')

    ax.set_xlim(-0.5, 6.5)
    ax.set_ylim(0, max(second_spfa) * 1.15)
...
def plot_combined():
    ...
    ax1.plot(nodes, shortest_spfa, 's-',
             color='#e74c3c', linewidth=2.5, markersize=8, label='SPFA')
    ...
    ax1.set_ylim(0, max(shortest_spfa) * 1.1)

    # 左图着色
    ax1.fill_between(nodes, 0, [max(shortest_spfa) * 1.1] * len(nodes),
                     alpha=0.08, color='#2ecc71')
    ax1.text(max(nodes) * 0.5, max(shortest_spfa) * 0.85,
    ...
    ax2.plot(nodes, second_spfa, 's-',
             color='#e67e22', linewidth=2.5, markersize=8, label='State-Extended SPFA')
    ...
    # 右图区域着色
    ax2.fill_between([0, crossover_nodes], 0, max(second_spfa) * 1.1,
                     alpha=0.08, color='#e67e22')
    ax2.fill_between([crossover_nodes, max(nodes) * 1.05], 0, max(second_spfa) * 1.1,
                     alpha=0.08, color='#3498db')

    ax2.set_xlim(0, max(nodes) * 1.05)
    ax2.set_ylim(0, max(second_spfa) * 1.1)
...
def plot_speedup_line():
    ...
    # 计算加速比：Dijkstra 时间 / SPFA 时间
    # > 1 表示 SPFA 更快，< 1 表示 Dijkstra 更快
    shortest_ratio = [s / d for d, s in zip(shortest_dijkstra, shortest_spfa)]
    second_ratio = [s / d for d, s in zip(second_dijkstra, second_spfa)]

    ax.plot(nodes, shortest_ratio, 'o-',
            color='#27ae60', linewidth=2.5, markersize=10,
            label='Shortest Path (SPFA/Dijkstra)')
    ax.plot(nodes, second_ratio, 's-',
            color='#e67e22', linewidth=2.5, markersize=10,
            label='Second Shortest Path (SPFA/Dijkstra)')
    ...
    ax.fill_between([0, max(nodes) * 1.05], 0, 1,
                    alpha=0.1, color='#e74c3c', label='_SPFA faster region')

    ax.text(max(nodes) * 0.7, 0.75, 'SPFA faster', ha='center', fontsize=11,
            color='#c0392b', fontweight='bold')
    ax.text(max(nodes) * 0.7, 2.2, 'Dijkstra faster', ha='center', fontsize=11,
            color='#2980b9', fontweight='bold')

    ax.set_xlabel('Number of Nodes', fontsize=12)
    ax.set_ylabel('Time Ratio (SPFA / Dijkstra)', fontsize=12)
    ax.set_title('Performance Ratio: SPFA vs Dijkstra on Grid Graphs\n(Ratio > 1 means Dijkstra is faster)',
