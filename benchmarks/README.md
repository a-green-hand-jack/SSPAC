# Benchmark: Dijkstra vs Queue-Optimized Bellman-Ford on Grid Graphs

This document presents a comprehensive benchmark comparing **Dijkstra** and **Queue-Optimized Bellman-Ford** algorithms for both **shortest path** and **second shortest path** problems on grid graphs.
...
Grid graphs are known to be adversarial cases for Queue-Optimized Bellman-Ford (also known as SPFA - Shortest Path Faster Algorithm). This benchmark investigates:

1. **Shortest Path Problem**: Does Queue-Optimized Bellman-Ford degrade on grid graphs as expected?
2. **Second Shortest Path Problem**: Does the same pattern hold for second shortest path algorithms?

### Why Grid Graphs?

Grid graphs have unique properties that stress-test Queue-Optimized Bellman-Ford:
...
These properties cause Queue-Optimized Bellman-Ford to repeatedly re-enqueue nodes, degrading its performance from the expected O(kM) to worst-case O(NM).
...
| **Queue-Optimized Bellman-Ford** | FIFO queue | O(kM) average, O(NM) worst |
...
| **State-Extended Bellman-Ford** | FIFO queue maintaining (d1, d2) | O(kM) average, O(NM) worst |
...
**Observation**: Dijkstra consistently outperforms Queue-Optimized Bellman-Ford on grid graphs across all tested sizes.

| Grid Size | Dijkstra (ms) | Bellman-Ford (ms) | Winner | Speedup |
|-----------|---------------|-------------------|--------|---------|
...
**Key Insight**: The performance gap widens as grid size increases. Queue-Optimized Bellman-Ford's enqueue count explodes (up to 10× more than Dijkstra at 400×400), causing severe performance degradation.
...
| Grid Size | Dijkstra (ms) | Bellman-Ford (ms) | Winner | Ratio |
|-----------|---------------|-------------------|--------|-------|
| 100 × 100 | 2.49 | 1.61 | **Bellman-Ford** | 0.65× |
| 150 × 150 | 5.96 | 4.31 | **Bellman-Ford** | 0.72× |
| 200 × 200 | 10.93 | 8.83 | **Bellman-Ford** | 0.81× |
...
- Below this threshold: Bellman-Ford is faster
...
| Problem | Bellman-Ford Advantage Region | Dijkstra Advantage Region |
|---------|------------------------------|--------------------------|
...
This chart shows the ratio of Bellman-Ford time to Dijkstra time:
- **Ratio > 1**: Dijkstra is faster
- **Ratio < 1**: Bellman-Ford is faster
...
2. **Second Shortest Path** (orange line):
   - Starts below 1.0 (Bellman-Ford faster)
   - Crosses 1.0 at ~62,500 nodes
   - Continues to increase (Dijkstra faster)

## Key Findings

### 1. Grid Graphs Expose Bellman-Ford's Weakness

On grid graphs, Queue-Optimized Bellman-Ford's worst-case behavior is triggered:
- Enqueue operations grow super-linearly with graph size
- At 400×400, Bellman-Ford enqueues 10× more nodes than Dijkstra
...
- Bellman-Ford's O(1) queue operations partially offset its re-enqueue overhead
...
| Second shortest path, small graphs (< 60K nodes) | **Bellman-Ford** |
| Second shortest path, large graphs (> 60K nodes) | **Dijkstra** |
| Sparse random graphs | Bellman-Ford (often faster) |
...
1. **Grid graphs are adversarial for Queue-Optimized Bellman-Ford** - confirming theoretical expectations
2. **The second shortest path problem has unique performance characteristics** - Bellman-Ford maintains an advantage at smaller scales
